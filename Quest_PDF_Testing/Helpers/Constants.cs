namespace Quest_PDF_Testing.Helpers
{
    public static class Constants
    {
        public static readonly float TITLE_FONT_SIZE_30 = 30f;
        public static readonly float TEXT_FONT_SIZE_10 = 10f;
        public static readonly float TEXT_FONT_SIZE_18 = 18f;
        public static readonly float CONTROL_FONT_SIZE_25 = 25f;
        public static readonly float HALF_POINT_PADDING = 0.5f;
        public static readonly float PADDING_1 = 1f;
        public static readonly float PADDING_2 = 2f;
        public static readonly float PADDING_3 = 3f;
        public static readonly float PADDING_5 = 5f;
        public static readonly float PADDING_10 = 10f;
        public static readonly float PADDING_15 = 15f;
        public static readonly float BORDER_1 = 1f;
        public static readonly float BORDER_2 = 2f;
        public static readonly float HEADER_IMAGE_INITIAL_MAX_HEIGHT = 100f;
        public static readonly float HEADER_IMAGE_INITIAL_MAX_WIDTH = 50f;
        public static readonly string GRAY_COLOR_HEX = "#c9c9c9";
        public static readonly string GRAY_COLOR_HEX_2 = "#827e7e";
        public static readonly string RED_COLOR_HEX = "#d60b3a";
        public static readonly string SUCCESS_COLOR_HEX = "#06c709";
        public static readonly string FAIL_COLOR_HEX = "#a6080b";
        public static readonly int MAX_COLUMNS_FOR_HIGHLIGHTED_ITEMS = 5;
    }
}
